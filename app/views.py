...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        """
        When messages are created via the API, automatically compute an embedding
        for their text so every message has a vector representation.
        """
        instance = serializer.save()
        instance.embedding = embed_text(instance.text or "")
        instance.save(update_fields=["embedding"])
 
    def perform_update(self, serializer):
        """
        When message text is edited via the API, refresh the embedding.
        """
        instance = serializer.save()
        instance.embedding = embed_text(instance.text or "")
        instance.save(update_fields=["embedding"])
    ...
    def regenerate(self, request, pk=None):
        ...
        assistant_message = Message.objects.create(
            ...
            text=ai_answer,
            embedding=embed_text(ai_answer or ""),
        )
...
def chat(request):
    ...
    try:
        # Embed the new user message once so we can both search, create
        # memory entries, and persist the embedding on the Message row
